...
**Goal:** Align the Pocket Nori web app’s visual presentation with a lighter, analytics-style dashboard while keeping all routing, data flow, and feature logic unchanged.
...
- Added migration `007_topic_clusters.sql`, creating `topic_clusters` and adding `cluster_id` to both `topics` and `topic_arcs` so Pocket Nori has a stored canonical topic layer instead of recomputing clusters on every read.
...
**Goal:** Replace the old commitments-only frontend with the new Actions terminology and flow from the updated root brief.

- Updated the main app shell so navigation now uses `Home` and `Actions`, while removing the older workspace/pilot-mode chrome in the same touched surface.
- Reworked the home dashboard to show `Actions` instead of `Commitments`, with separate `Commitments` and `Follow-ups` sections sourced from the new `action_type` filter.
- Added frontend API support for `action_type` on `GET /commitments` and manual `POST /commitments` creation.
- Rebuilt `/commitments` into an `Actions` surface with two tabs (`Commitments` and `Follow-ups`), simplified one-card-per-item layout, resolve-in-place behavior, and a manual creation form.
...
- Continue with Wave D from `CODEX_BRIEF.md` (`You` profile dropdown + entities navigation restructure)

---

## 2026-03-16 — Wave D frontend: profile dropdown + entity management

**Goal:** Restructure navigation around a top-right personal menu and turn the entity directory into a management surface.

- Removed `Entities` from primary navigation and added a `You`-style profile menu in the app header using session-derived initials and display name.
- Added dropdown items for profile summary, `Manage Entities`, disabled `Language` / `Help Center` placeholders, and `Sign out`.
- Reworked `/entities` into `Manage Entities` with search, type filters, sort controls, and browser-local display-label editing.
- Kept entity label edits explicitly local to the browser because the current backend only exposes read-only grouped entity summaries with no stable IDs or mutation route.
...
- Proceed to Wave E from the brief (meeting detail tab navigation + transcript conversation view + Actions naming alignment)

---

## 2026-03-16 — Wave E frontend: meeting detail overhaul

**Goal:** Rework the meeting detail page around tabbed navigation, clearer meeting metadata, and a transcript conversation view.

- Rebuilt meeting detail into three tabs: `Topics`, `Actions`, and `Transcript`.
- Cleaned up the meeting header with an `Online` tag, timezone-name-free date display, topic/action/connection count chips, and the existing brief deep link.
- Added a dedicated connections column in the `Topics` tab using `GET /conversations/{id}/connections`, with `No connections yet.` empty-state fallback.
- Replaced the old transcript segment dump with a grouped conversation-style transcript view built from consecutive speaker turns.
- Removed the inline `Entities` section from meeting detail so the page now points entity management to the dedicated `/entities` surface.
- Renamed the commitments section to `Actions`; per-meeting cards are not yet split into commitments vs follow-ups because the current conversation-detail API still omits `action_type`.
...
- Proceed to Wave F from the brief (persistent global search bar)

---

## 2026-03-16 — Wave F frontend: persistent global search bar

**Goal:** Make search available from the shared shell and keep `/search` aligned with URL-driven navigation.

- Added a persistent header search form to the shared app frame so users can launch search from any screen.
- Updated `/search` to read the `q` query parameter, preload the matching query into the page input, and automatically run a `Find` search for URL-driven navigations.
- Kept manual `/search` submissions in sync with the URL so the header search field and search page stay aligned without breaking `Ask` mode.
- Wrapped the shared shell in `Suspense` to satisfy Next.js prerender requirements for `useSearchParams`.
...
- Proceed to Wave G from the brief

---

## 2026-03-16 — Wave E follow-up: meeting detail actions split by type

**Goal:** Finish the deferred Wave E action split now that `GET /conversations/{id}` returns `action_type`.

- Added `action_type` to the meeting-detail conversation contract in the frontend API layer.
- Updated the meeting detail `Actions` tab to split items into `Commitments` and `Follow-ups` instead of rendering a single flat list.
- Removed the old placeholder note that said per-meeting follow-up classification was waiting on backend support.
...
- Proceed to Wave G from the brief, or run the compact QA pass across Search, Topics, Dashboard, and Actions

---

## 2026-03-16 — Wave G frontend: persistent import entry on Meetings

**Goal:** Keep meeting import available from the Meetings page at all times, not only during first-run onboarding.

- Added a persistent `Import past meetings` entry point at the top of the Meetings page above the indexed meeting list.
- Wired the entry point directly to `/onboarding`.
- Kept the styling understated as a soft secondary action row so it stays available without competing with the meeting cards.
- Left it unconditional so users can return to import more recordings at any time.
